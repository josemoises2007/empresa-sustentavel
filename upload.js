  document.addEventListener('DOMContentLoaded', () => {
            fetch('https://ajudapratodos.000webhostapp.com/missoes/fetch_uploads.php')
                    ...
                    Object.values(groups).forEach(group => {
                        ...
                        group.elements.forEach(element => {
                            if (element.file_type.startsWith('image/')) {
                                fileElements += `
                                    <div class="media">
                                        <img src="uploaded_files/${element.file_name}" alt="${element.file_name}">
                                        <div class="file-info">
                                            <a href="uploaded_files/${element.file_name}" target="_blank">${element.file_name}</a>
                                        </div>
                                    </div>
                                `;
                                ...
                                fileElements += `
                                    <div class="media">
                                        <object data="uploaded_files/${element.file_name}" ></object>
                                        <div class="file-info">
                                            <a href="uploaded_files/${element.file_name}" target="_blank">${element.file_name}</a>
                                        </div>
                                    </div>
                                ...
                                fileElements += `
                                    <div class="media">
                                        <video src="uploaded_files/${element.file_name}" controls></video>
                                        <div class="file-info">
                                            <a href="uploaded_files/${element.file_name}" target="_blank">${element.file_name}</a>
                                        </div>
                                    </div>
                        ...
                        container.innerHTML = `
                            <div class="group-info">
                                <p><strong>Autor:</strong> ${group.author}</p>
                                <p><strong>Obra:</strong> ${group.title}</p>
                                ${fileElements}
                            </div>
                        `;
                ...
                .catch(error => console.error('Erro ao buscar uploads:', error));
        });
        ...
        function uploadFiles() {
            ...
            fetch('https://ajudapratodos.000webhostapp.com/missoes/upload.php', {
                ...
                .then(data => {
                    alert(data.message);
                    if (data.message.startsWith('Arquivos enviados')) {
                        location.reload();
                    }
                })
